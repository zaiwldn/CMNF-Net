from dataset import *
import evaluation
from cmnf import CMNF
from trainer import Trainer
from optparse import OptionParser
import warnings
warnings.filterwarnings('ignore')
from dataset.dutsv2 import TrainDUTSv2
from dataset.ytvos import TrainYTVOS

print("PyTorch_ver:", torch.__version__)
...
def train_ytvos(model, ver):
    train_set = TrainYTVOS('./dataset/YTOBJ_train', 'train', clip_n=512)
    train_loader = torch.utils.data.DataLoader(train_set, batch_size=6, shuffle=True, num_workers=4, pin_memory=True)
    val_set = TestDAVIS('./dataset/DAVIS', '2016', 'myval')
    ...
    trainer = Trainer(model, ver, optimizer, train_loader, val_set, save_name='train_pre', save_step=1000,val_step=100, scheduler=scheduler)
    trainer.train(3000)

def train_duts_davis(model, ver):
    dutsv2_set = TrainDUTSv2('./dataset/DUTSv2', clip_n=384)
    davis_set = TrainDAVIS('./dataset/DAVIS', '2016', 'mytrain', clip_n=128)
    ...
    trainer = Trainer(model, ver, optimizer, train_loader, val_set, save_name='train_last', save_step=1000, val_step=100, scheduler=scheduler)
    trainer.train(4000)
...
def test_davis(model):
    evaluator = evaluation.Evaluator(TestDAVIS('./dataset/DAVIS', '2016', 'myval'))
    evaluator.evaluate(model, os.path.join('outputs', 'DAVIS16_val'))


def test_fbms(model):
    test_set = TestFBMS('./dataset/FBMS/TestSet')
...
def test_ytobj(model):
    test_set = TestYTOBJ('./dataset//YTOBJ')
    test_loader = torch.utils.data.DataLoader(test_set, batch_size=1, num_workers=4)
    model.cuda()
...
def test_lvid(model):
    test_set = TestLVID('./dataset/LVID')
    test_loader = torch.utils.data.DataLoader(test_set, batch_size=1, num_workers=4)
    model.cuda()
    ious = []
...
if __name__ == '__main__':
    ...
    ver = 'mitb2'
    aos = False
    model = CMNF(ver, aos).eval()


    # training stage
    if options.train:
        # model = torch.nn.DataParallel(model)
        train_ytvos(model, ver)
        train_duts_davis(model, ver)

    # testing stage
    if options.test:
        model.load_state_dict(torch.load('weights/train_last_best.pth'.format(ver), map_location='cpu'))
        with torch.no_grad():
            test_davis(model)
            test_fbms(model)
            test_ytobj(model)
