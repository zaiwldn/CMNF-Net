...
class Trainer(object):
    def __init__(self, model, ver, optimizer, train_loader, val_set, save_name, save_step, val_step, scheduler=None):
            ...
            for video_name, video_parts in self.val_set.get_videos():
                for vos_data in video_parts:
                    ...
                    if 0 in object_ids:
                        object_ids.remove(0)
                    if not object_ids:
                        print(f"跳过无目标的视频片段: {video_name}")
                        continue

                    all_res_masks = numpy.zeros((len(object_ids), L, H, W))
                    all_gt_masks = numpy.zeros((len(object_ids), L, H, W))
                    for idx, k in enumerate(object_ids):
                        res_masks_k = res_masks.cpu().numpy().copy()
                        res_masks_k[res_masks_k != k] = 0
                        res_masks_k[res_masks_k != 0] = 1
                        all_res_masks[idx] = res_masks_k.squeeze()

                        gt_masks_k = gt_masks.cpu().numpy().copy()
                        gt_masks_k[gt_masks_k != k] = 0
                        gt_masks_k[gt_masks_k != 0] = 1
                        all_gt_masks[idx] = gt_masks_k.squeeze()

                    for i in range(len(object_ids)):
                        j_metrics = metrics.db_eval_iou(all_gt_masks[i], all_res_masks[i])
                        f_metrics = metrics.db_eval_boundary(all_gt_masks[i], all_res_masks[i])
                        metrics_res['J'].extend(j_metrics)
                        metrics_res['F'].extend(f_metrics)

                    # print(f"[{video_name}] JMetrics: {numpy.mean(j_metrics):.4f}")
                    # print(f"[{video_name}] FMetrics: {numpy.mean(f_metrics):.4f}")
