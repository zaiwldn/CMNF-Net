...
def db_eval_iou(annotation, segmentation):
    ...
    annotation = annotation.astype(bool)  # 修改这里
    segmentation = segmentation.astype(bool)  # 修改这里
...
def db_eval_boundary(annotation, segmentation, bound_th=0.008):
    if annotation.ndim == 3:
        ...
        raise ValueError("Annotation must be 2D or 3D")
    return f_res
...
def _seg2bmap(seg, width=None, height=None):
    ...
    if w == width and h == height:
        ...
        for x in range(w):
            for y in range(h):
                if b[y, x]:
                    j = 1 + int((y - 1) * height / h)
                    i = 1 + int((x - 1) * width / h)
                    bmap[j, i] = 1
    return bmap.astype(np.uint8)
